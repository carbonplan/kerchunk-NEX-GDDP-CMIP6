...
def main():
    ...
    catalog_url = "s3://carbonplan-share/nasa-nex-reference/reference_catalog_prod.csv"
    prune_bool = False

    for index, row in unique_df.iterrows():
        file_pattern = df.query(
            f"GCM == '{row['GCM']}'  & scenario == '{row['scenario']}'"
        )
        ...
        if not exists_bool:
            print(f"{GCM}_{scenario} is missing")
            if prune_bool:
                file_pattern = file_pattern[0:2]
            fsspec.filesystem("s3", anon=True, skip_instance_cache=True)
            dict(
                mode="rb",
                anon=True,
            ...
            tasks = [
                dask.delayed(generate_json_reference)(fil, temp_dir)
                for fil in file_pattern["url"]
            ]
            dask.compute(tasks)
            ...
            cat_df.loc[-1] = [
                store_name,
                os.path.join(target_root, store_name, output_file_name),
            ]
            cat_df.reset_index().drop(["index"], axis=1).to_csv(
                catalog_url, index=False
            )
